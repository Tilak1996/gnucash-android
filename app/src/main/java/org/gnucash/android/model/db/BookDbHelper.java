...
public class BookDbHelper extends SQLiteOpenHelper {
     ...
     */
    private static final String BOOKS_TABLE_CREATE = "CREATE TABLE " + DatabaseSchema.BookEntry.TABLE_NAME + " ("
            + DatabaseSchema.BookEntry._ID 		         + " integer primary key autoincrement, "
            + DatabaseSchema.BookEntry.COLUMN_UID 		     + " varchar(255) not null UNIQUE, "
            + DatabaseSchema.BookEntry.COLUMN_DISPLAY_NAME  + " varchar(255) not null, "
            + DatabaseSchema.BookEntry.COLUMN_ROOT_GUID     + " varchar(255) not null, "
            + DatabaseSchema.BookEntry.COLUMN_TEMPLATE_GUID + " varchar(255), "
            + DatabaseSchema.BookEntry.COLUMN_ACTIVE        + " tinyint default 0, "
            + DatabaseSchema.BookEntry.COLUMN_SOURCE_URI    + " varchar(255), "
            + DatabaseSchema.BookEntry.COLUMN_LAST_SYNC     + " TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, "
            + DatabaseSchema.BookEntry.COLUMN_CREATED_AT    + " TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, "
...
//                Crashlytics.log(err_msg);
        ...
        String sql = "SELECT COUNT(*) FROM " + DatabaseSchema.BookEntry.TABLE_NAME;
    ...
    private void insertBook(SQLiteDatabase db, Book book) {
        ...
        contentValues.put(DatabaseSchema.BookEntry.COLUMN_UID, book.getUID());
        ...
        contentValues.put(DatabaseSchema.BookEntry.COLUMN_DISPLAY_NAME, new BooksDbAdapter(db).generateDefaultBookName());
        contentValues.put(DatabaseSchema.BookEntry.COLUMN_ACTIVE, book.isActive() ? 1 : 0);

        db.insert(DatabaseSchema.BookEntry.TABLE_NAME, null, contentValues);
    }
